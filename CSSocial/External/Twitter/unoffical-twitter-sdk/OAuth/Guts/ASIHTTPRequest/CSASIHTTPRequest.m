...
//  http://allseeing-i.com/CSASIHTTPRequest
//
...
#import "CSASIAuthenticationDialog.h"
...
NSString *CSASIHTTPRequestVersion = @"v1.7-25 2010-07-22";

NSString* const CSNetworkRequestErrorDomain = @"CSASIHTTPRequestErrorDomain";

static NSString *CSASIHTTPRequestRunLoopMode = @"CSASIHTTPRequestRunLoopMode";
...
static void ReadStreamClientCallBack(CFReadStreamRef readStream, CFStreamEventType type, void *clientCallBackInfo) {
    [((CSASIHTTPRequest*)clientCallBackInfo) handleNetworkEvent: type];
}
...
static NSError *CSASIRequestCancelledError;
static NSError *CSASIRequestTimedOutError;
static NSError *CSASIAuthenticationError;
static NSError *CSASIUnableToCreateRequestError;
static NSError *CSASITooMuchRedirectionError;
...
unsigned long const CSASIWWANBandwidthThrottleAmount = 14800;
...
// When using CSASIAuthenticationDialogs, it also ensures only one dialog is shown at once
...
static id <CSASICacheDelegate> defaultCache = nil;
...
#if TARGET_OS_IPHONE
// Use [CSASIHTTPRequest setShouldUpdateNetworkActivityIndicator:NO] if you want to manage it yourself
...
@property (assign) CSASIAuthenticationState authenticationNeeded;
...
@implementation CSASIHTTPRequest

#pragma mark init / dealloc

+ (void)initialize
{
	if (self == [CSASIHTTPRequest class]) {
		...
		CSASIRequestTimedOutError = [[NSError errorWithDomain:CSNetworkRequestErrorDomain code:CSASIRequestTimedOutErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"The request timed out",NSLocalizedDescriptionKey,nil]] retain];	
		CSASIAuthenticationError = [[NSError errorWithDomain:CSNetworkRequestErrorDomain code:CSASIAuthenticationErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Authentication needed",NSLocalizedDescriptionKey,nil]] retain];
		CSASIRequestCancelledError = [[NSError errorWithDomain:CSNetworkRequestErrorDomain code:CSASIRequestCancelledErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"The request was cancelled",NSLocalizedDescriptionKey,nil]] retain];
		CSASIUnableToCreateRequestError = [[NSError errorWithDomain:CSNetworkRequestErrorDomain code:CSASIUnableToCreateRequestErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Unable to create request (bad url?)",NSLocalizedDescriptionKey,nil]] retain];
		CSASITooMuchRedirectionError = [[NSError errorWithDomain:CSNetworkRequestErrorDomain code:CSASITooMuchRedirectionErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"The request failed because it redirected too many times",NSLocalizedDescriptionKey,nil]] retain];	
...
- (id)initWithURL:(NSURL *)newURL
{
	...
	[self setTimeOutSeconds:[CSASIHTTPRequest defaultTimeOutSeconds]];
...
+ (id)requestWithURL:(NSURL *)newURL usingCache:(id <CSASICacheDelegate>)cache
{
	return [self requestWithURL:newURL usingCache:cache andCachePolicy:CSASIDefaultCachePolicy];
}

+ (id)requestWithURL:(NSURL *)newURL usingCache:(id <CSASICacheDelegate>)cache andCachePolicy:(CSASICachePolicy)policy
{
	CSASIHTTPRequest *request = [[[self alloc] initWithURL:newURL] autorelease];
...
- (void)dealloc
{
	[self setAuthenticationNeeded:CSASINoAuthenticationNeededYet];
...
- (void)buildPostBody
{
	...
	if ([self postBodyFilePath]) {
		...
		if ([self shouldCompressRequestBody]) {
			...
			[CSASIHTTPRequest compressDataFromFile:[self postBodyFilePath] toFile:[self compressedPostBodyFilePath]];
		...
		if (err) {
			[self failWithError:[NSError errorWithDomain:CSNetworkRequestErrorDomain code:CSASIFileManagementError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Failed to get attributes for file at path '%@'",path],NSLocalizedDescriptionKey,error,NSUnderlyingErrorKey,nil]]];
			return;
		}
	...
	// Otherwise, we have an in-memory request body
		...
		if ([self shouldCompressRequestBody]) {
			[self setCompressedPostBody:[CSASIHTTPRequest compressData:[self postBody]]];
...
- (void)cancel
{
	...
	[self failWithError:CSASIRequestCancelledError];
...
{	
	if ([self isResponseCompressed]) {
		return [CSASIHTTPRequest uncompressZippedData:[self rawResponseData]];
	} else {
...
#endif
	[self setRunLoopMode:CSASIHTTPRequestRunLoopMode];
...
- (void)main
{
	@try {
		...
		// A HEAD request generated by an CSASINetworkQueue may have set the error already. If so, we should not proceed.
		if ([self error]) {
			[self setComplete:YES];
			[self markAsFinished];
			return;		
		...
		if (![self url]) {
			[self failWithError:CSASIUnableToCreateRequestError];
			return;		
		}
		...
		//If this is a HEAD request generated by an CSASINetworkQueue, we need to let the main request generate its headers first so we can use them
		if ([self mainRequest]) {
			[[self mainRequest] buildRequestHeaders];
		}
		
		// Even if this is a HEAD request with a mainRequest, we still need to call to give subclasses a chance to add their own to HEAD requests (CSASIS3Request does this)
		[self buildRequestHeaders];
		
		if ([self downloadCache]) {
			if ([self cachePolicy] == CSASIDefaultCachePolicy) {
			...
			if ([self cachePolicy] == CSASIOnlyLoadIfNotCachedCachePolicy) {
				if ([self useDataFromCache]) {
					return;
				}
			} else if ([self cachePolicy] == CSASIReloadIfDifferentCachePolicy) {
		...
		NSError *underlyingError = [NSError errorWithDomain:CSNetworkRequestErrorDomain code:CSASIUnhandledExceptionError userInfo:[exception userInfo]];
		[self failWithError:[NSError errorWithDomain:CSNetworkRequestErrorDomain code:CSASIUnhandledExceptionError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[exception name],NSLocalizedDescriptionKey,[exception reason],NSLocalizedFailureReasonErrorKey,underlyingError,NSUnderlyingErrorKey,nil]]];
...
- (void)buildRequestHeaders
{
	...
	if (![[self requestHeaders] objectForKey:@"User-Agent"]) {
		NSString *userAgentString = [CSASIHTTPRequest defaultUserAgentString];
		if (userAgentString) {
...
- (void)updatePartialDownloadSize
{
	if ([self allowResumeForFileDownloads] && [self downloadDestinationPath] && [self temporaryFileDownloadPath] && [[NSFileManager defaultManager] fileExistsAtPath:[self temporaryFileDownloadPath]]) {
		...
		if (err) {
			[self failWithError:[NSError errorWithDomain:CSNetworkRequestErrorDomain code:CSASIFileManagementError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Failed to get attributes for file at path '%@'",[self temporaryFileDownloadPath]],NSLocalizedDescriptionKey,error,NSUnderlyingErrorKey,nil]]];
			return;
		}
	}
}
...
- (void)startRequest
{
	...
	if ([self shouldStreamPostDataFromDisk] && [self postBodyFilePath] && [[NSFileManager defaultManager] fileExistsAtPath:[self postBodyFilePath]]) {
		...
		if ([self compressedPostBodyFilePath] && [[NSFileManager defaultManager] fileExistsAtPath:[self compressedPostBodyFilePath]]) {
			[self setPostBodyReadStream:[CSASIInputStream inputStreamWithFileAtPath:[self compressedPostBodyFilePath] request:self]];
		} else {
			[self setPostBodyReadStream:[CSASIInputStream inputStreamWithFileAtPath:[self postBodyFilePath] request:self]];
		}
		...
		if ([self postBody] && [[self postBody] length] > 0) {
			if ([self shouldCompressRequestBody] && [self compressedPostBody]) {
				[self setPostBodyReadStream:[CSASIInputStream inputStreamWithData:[self compressedPostBody] request:self]];
			} else if ([self postBody]) {
				[self setPostBodyReadStream:[CSASIInputStream inputStreamWithData:[self postBody] request:self]];
			}
			[self setReadStream:[(NSInputStream *)CFReadStreamCreateForStreamedHTTPRequest(kCFAllocatorDefault, request,(CFReadStreamRef)[self postBodyReadStream]) autorelease]];
	...
	if (![self readStream]) {
		[self failWithError:[NSError errorWithDomain:CSNetworkRequestErrorDomain code:CSASIInternalErrorWhileBuildingRequestType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Unable to create read stream",NSLocalizedDescriptionKey,nil]]];
        return;
    }
	...
	if (![self proxyHost] && ![self proxyPort]) {
		...
		if ([self PACurl]) {
			...
			proxies = [CSASIHTTPRequest proxiesForURL:[self url] fromPAC:[self PACurl]];
			...
			if ([settings objectForKey:(NSString *)kCFProxyAutoConfigurationURLKey]) {
				proxies = [CSASIHTTPRequest proxiesForURL:[self url] fromPAC:[settings objectForKey:(NSString *)kCFProxyAutoConfigurationURLKey]];
			}
		}
		
		if (!proxies) {
			[self setReadStream:nil];
			[self failWithError:[NSError errorWithDomain:CSNetworkRequestErrorDomain code:CSASIInternalErrorWhileBuildingRequestType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Unable to obtain information on proxy servers needed for request",NSLocalizedDescriptionKey,nil]]];
			return;			
		}
	...
	[CSASIHTTPRequest expirePersistentConnections];
	...
	if ([self shouldAttemptPersistentConnection]) {
		...
		CFReadStreamSetProperty((CFReadStreamRef)[self readStream], CFSTR("CSASIStreamID"), [[self connectionInfo] objectForKey:@"id"]);
   ...
   // Start the HTTP connection
	...
	if (!streamSuccessfullyOpened) {
		...
		[self failWithError:[NSError errorWithDomain:CSNetworkRequestErrorDomain code:CSASIInternalErrorWhileBuildingRequestType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Unable to start HTTP connection",NSLocalizedDescriptionKey,nil]]];
		return;	
	}
	...
	if (![self mainRequest]) {
		if ([self shouldResetUploadProgress]) {
			...
			[CSASIHTTPRequest updateProgressIndicator:&uploadProgressDelegate withProgress:0 ofTotal:1];
		}
		if ([self shouldResetDownloadProgress] && ![self partialDownloadSize]) {
			[CSASIHTTPRequest updateProgressIndicator:&downloadProgressDelegate withProgress:0 ofTotal:1];
		}
	}	
...
- (void)performRedirect
{
	...
	if ([self redirectCount] > RedirectionLimit) {
		...
		[self failWithError:CSASITooMuchRedirectionError];
		[self setComplete:YES];
	} else {
...
- (void)checkRequestStatus
{
	...
	if ([self shouldTimeOut]) {			
		...
		[self failWithError:CSASIRequestTimedOutError];
		[self cancelLoad];
		[self setComplete:YES];
		[[self cancelledLock] unlock];
		return;
	...
	if ([self readStream]) {
		...
		if ([self postLength]) {
			...
			if (totalBytesSent > lastBytesSent) {
				...
				[self setLastActivityTime:[NSDate date]];
				[CSASIHTTPRequest incrementBandwidthUsedInLastSecond:(unsigned long)(totalBytesSent-lastBytesSent)];		
...
- (void)removeTemporaryDownloadFile
{
	if ([self temporaryFileDownloadPath]) {
		if ([[NSFileManager defaultManager] fileExistsAtPath:[self temporaryFileDownloadPath]]) {
			...
			if (removeError) {
				[self failWithError:[NSError errorWithDomain:CSNetworkRequestErrorDomain code:CSASIFileManagementError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Failed to delete file at path '%@'",[self temporaryFileDownloadPath]],NSLocalizedDescriptionKey,removeError,NSUnderlyingErrorKey,nil]]];
			}
		}
...
- (void)removePostDataFile
{
	if ([self postBodyFilePath]) {
		...
		if (removeError) {
			[self failWithError:[NSError errorWithDomain:CSNetworkRequestErrorDomain code:CSASIFileManagementError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Failed to delete file at path '%@'",[self postBodyFilePath]],NSLocalizedDescriptionKey,removeError,NSUnderlyingErrorKey,nil]]];
		}
	...
	if ([self compressedPostBodyFilePath]) {
		...
		if (removeError) {
			[self failWithError:[NSError errorWithDomain:CSNetworkRequestErrorDomain code:CSASIFileManagementError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Failed to delete file at path '%@'",[self compressedPostBodyFilePath]],NSLocalizedDescriptionKey,removeError,NSUnderlyingErrorKey,nil]]];
		}
...
// Used by CSASINetworkQueue to create a HEAD request appropriate for this request with the same headers (though you can use it yourself)
- (CSASIHTTPRequest *)HEADRequest
{
	CSASIHTTPRequest *headRequest = [[self class] requestWithURL:[self url]];
...
- (void)setUploadProgressDelegate:(id)newDelegate
{
	...
	double max = 1.0;
	[CSASIHTTPRequest performSelector:@selector(setMaxValue:) onTarget:&uploadProgressDelegate withObject:nil amount:&max];
	#endif
	[[self cancelledLock] unlock];
}
...
- (void)setDownloadProgressDelegate:(id)newDelegate
{
	...
	double max = 1.0;
	[CSASIHTTPRequest performSelector:@selector(setMaxValue:) onTarget:&downloadProgressDelegate withObject:nil amount:&max];	
	#endif
	[[self cancelledLock] unlock];
}
...
- (void)updateDownloadProgress
{
	...
	[CSASIHTTPRequest performSelector:@selector(request:didReceiveBytes:) onTarget:&queue withObject:self amount:&value];
	[CSASIHTTPRequest performSelector:@selector(request:didReceiveBytes:) onTarget:&downloadProgressDelegate withObject:self amount:&value];
	[CSASIHTTPRequest updateProgressIndicator:&downloadProgressDelegate withProgress:[self totalBytesRead]+[self partialDownloadSize] ofTotal:[self contentLength]+[self partialDownloadSize]];
...
- (void)updateUploadProgress
{
	...
	[CSASIHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&queue withObject:self amount:&value];
	[CSASIHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&uploadProgressDelegate withObject:self amount:&value];
	[CSASIHTTPRequest updateProgressIndicator:&uploadProgressDelegate withProgress:[self totalBytesSent]-[self uploadBufferSize] ofTotal:[self postLength]-[self uploadBufferSize]];
}


- (void)incrementDownloadSizeBy:(long long)length
{
	[CSASIHTTPRequest performSelector:@selector(request:incrementDownloadSizeBy:) onTarget:&queue withObject:self amount:&length];
	[CSASIHTTPRequest performSelector:@selector(request:incrementDownloadSizeBy:) onTarget:&downloadProgressDelegate withObject:self amount:&length];
}


- (void)incrementUploadSizeBy:(long long)length
{
	[CSASIHTTPRequest performSelector:@selector(request:incrementUploadSizeBy:) onTarget:&queue withObject:self amount:&length];
	[CSASIHTTPRequest performSelector:@selector(request:incrementUploadSizeBy:) onTarget:&uploadProgressDelegate withObject:self amount:&length];
}
...
-(void)removeUploadProgressSoFar
{
	...
	[CSASIHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&queue withObject:self amount:&progressToRemove];
	[CSASIHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&uploadProgressDelegate withObject:self amount:&progressToRemove];
	[CSASIHTTPRequest updateProgressIndicator:&uploadProgressDelegate withProgress:0 ofTotal:[self postLength]];
}
...
+ (void)performSelector:(SEL)selector onTarget:(id *)target withObject:(id)object amount:(void *)amount
{
	if ([*target respondsToSelector:selector]) {
        ...
        NSMethodSignature *cbSignature = [CSASIHTTPRequest methodSignatureForSelector:callback];
...
+ (void)updateProgressIndicator:(id *)indicator withProgress:(unsigned long long)progress ofTotal:(unsigned long long)total
{
	...
	[progressLock lock];
	[CSASIHTTPRequest performSelector:selector onTarget:indicator withObject:nil amount:&progressAmount];
	[progressLock unlock];
}
...
- (void)callSelectorCallback:(SEL *)selectorPtr withTarget:(id *)targetPtr request:(CSASIHTTPRequest *)request
{
	id target = *targetPtr;
...
- (void)callSelectorOnMainThread:(SEL *)selector forDelegate:(id *)target
{
	...
	NSMethodSignature *signature = [CSASIHTTPRequest instanceMethodSignatureForSelector:callback];
...
{
#if DEBUG_REQUEST_STATUS || DEBUG_THROTTLING
	NSLog(@"Request %@: %@",self,(theError == CSASIRequestCancelledError ? @"Cancelled" : @"Failed"));
#endif
	[self setComplete:YES];
	
	// Invalidate the current connection so subsequent requests don't attempt to reuse it
	if (theError && [theError code] != CSASIAuthenticationErrorType && [theError code] != CSASITooMuchRedirectionErrorType) {
		[connectionsLock lock];
		#if DEBUG_PERSISTENT_CONNECTIONS
	...
	if ([self downloadCache] && [self cachePolicy] == CSASIUseCacheIfLoadFailsCachePolicy) {
		if ([self useDataFromCache]) {
			return;
		}
	}
	...
	CSASIHTTPRequest *failedRequest = self;
	
	// If this is a HEAD request created by an CSASINetworkQueue or compatible queue delegate, make the main request fail
	if ([self mainRequest]) {
...
- (void)readResponseHeaders
{
	...
	if ([self downloadCache] && [self cachePolicy] == CSASIReloadIfDifferentCachePolicy) {
		if ([self useDataFromCache]) {
			CFRelease(message);
			return;
		}
	...
	if ([self responseStatusCode] == 401) {
		[self setAuthenticationNeeded:CSASIHTTPAuthenticationNeeded];
	} else if ([self responseStatusCode] == 407) {
		[self setAuthenticationNeeded:CSASIProxyAuthenticationNeeded];
	}
	...
	if ([self useCookiePersistence]) {
		...
		for (cookie in newCookies) {
			[CSASIHTTPRequest addSessionCookie:cookie];
		}
	}
	
	// Do we need to redirect?
	// Note that CSASIHTTPRequest does not currently support 305 Use Proxy
	...
	if (![self needsRedirect]) {
		...
		CSASIHTTPRequest *theRequest = self;
		if ([self mainRequest]) {
...
- (void)saveProxyCredentialsToKeychain:(NSDictionary *)newCredentials
{
	...
	if (authenticationCredentials) {
		[CSASIHTTPRequest saveCredentials:authenticationCredentials forProxy:[self proxyHost] port:[self proxyPort] realm:[self proxyAuthenticationRealm]];
	}	
}
...
- (void)saveCredentialsToKeychain:(NSDictionary *)newCredentials
{
	...
	if (authenticationCredentials) {
		[CSASIHTTPRequest saveCredentials:authenticationCredentials forHost:[[self url] host] port:[[[self url] port] intValue] protocol:[[self url] scheme] realm:[self authenticationRealm]];
	}	
}
...
- (NSMutableDictionary *)findProxyCredentials
{
	...
	// If this is a HEAD request generated by an CSASINetworkQueue, we'll try to use the details from the main request
	...
	if ((!user || !pass)) {
		NSURLCredential *authenticationCredentials = [CSASIHTTPRequest savedCredentialsForProxy:[self proxyHost] port:[self proxyPort] protocol:[[self url] scheme] realm:[self proxyAuthenticationRealm]];
		if (authenticationCredentials) {
...
- (NSMutableDictionary *)findCredentials
{
	...
	if ((!user || !pass) && useKeychainPersistence) {
		NSURLCredential *authenticationCredentials = [CSASIHTTPRequest savedCredentialsForHost:[[self url] host] port:[[[self url] port] intValue] protocol:[[self url] scheme] realm:[self authenticationRealm]];
		if (authenticationCredentials) {
			user = [authenticationCredentials user];
			pass = [authenticationCredentials password];
		}
...
{
	[self failWithError:CSASIAuthenticationError];
}
...
// Mac authentication dialog coming soon!
	...
	if ([self shouldPresentProxyAuthenticationDialog]) {
		[CSASIAuthenticationDialog performSelectorOnMainThread:@selector(presentAuthenticationDialogForRequest:) withObject:self waitUntilDone:[NSThread isMainThread]];
		return YES;
	}
	return NO;
#else
...
- (void)attemptToApplyProxyCredentialsAndResume
{
	...
	if (!proxyAuthentication) {
		[self cancelLoad];
		[self failWithError:[NSError errorWithDomain:CSNetworkRequestErrorDomain code:CSASIInternalErrorWhileApplyingCredentialsType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Failed to get authentication object from response headers",NSLocalizedDescriptionKey,nil]]];
		return;
	}
	...
	if (proxyCredentials) {
		...
		// Something went wrong, we'll have to give up
			...
			[self failWithError:[NSError errorWithDomain:CSNetworkRequestErrorDomain code:CSASIInternalErrorWhileApplyingCredentialsType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Failed to apply proxy credentials to request",NSLocalizedDescriptionKey,nil]]];
		}
...
- (void)attemptToApplyCredentialsAndResume
{
	...
	if ([self authenticationNeeded] == CSASIProxyAuthenticationNeeded) {
		[self attemptToApplyProxyCredentialsAndResume];
		return;
	}
	...
	if (requestCredentials) {
		...
		if ((([self authenticationScheme] != (NSString *)kCFHTTPAuthenticationSchemeNTLM) || [self authenticationRetryCount] < 2) && [self applyCredentials:requestCredentials]) {
			...
			[self failWithError:[NSError errorWithDomain:CSNetworkRequestErrorDomain code:CSASIInternalErrorWhileApplyingCredentialsType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Failed to apply credentials to request",NSLocalizedDescriptionKey,nil]]];
		}
...
{
	[self addRequestHeader:@"Authorization" value:[NSString stringWithFormat:@"Basic %@",[CSASIHTTPRequest base64forData:[[NSString stringWithFormat:@"%@:%@",theUsername,thePassword] dataUsingEncoding:NSUTF8StringEncoding]]]];	
}
...
- (void)handleBytesAvailable
{
	...
	// If zero bytes were read, wait for the EOF to come.
		...
		[CSASIHTTPRequest incrementBandwidthUsedInLastSecond:bytesRead];
...
{	
	...
	if ([self temporaryFileDownloadPath]) {
		...
		// Decompress the file (if necessary) directly to the destination path
			...
			int decompressionStatus = [CSASIHTTPRequest uncompressZippedDataFromFile:[self temporaryFileDownloadPath] toFile:[self downloadDestinationPath]];
			if (decompressionStatus != Z_OK) {
				fileError = [NSError errorWithDomain:CSNetworkRequestErrorDomain code:CSASIFileManagementError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Decompression of %@ failed with code %d",[self temporaryFileDownloadPath],decompressionStatus],NSLocalizedDescriptionKey,nil]];
			}
			[self removeTemporaryDownloadFile];
		} else {
			...
			if ([[NSFileManager defaultManager] fileExistsAtPath:[self downloadDestinationPath]]) {
				...
				if (moveError) {
					fileError = [NSError errorWithDomain:CSNetworkRequestErrorDomain code:CSASIFileManagementError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Unable to remove file at path '%@'",[self downloadDestinationPath]],NSLocalizedDescriptionKey,moveError,NSUnderlyingErrorKey,nil]];
				}
			}
			...
			if (!fileError) {
				...
				if (moveError) {
					fileError = [NSError errorWithDomain:CSNetworkRequestErrorDomain code:CSASIFileManagementError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Failed to move file from '%@' to '%@'",[self temporaryFileDownloadPath],[self downloadDestinationPath]],NSLocalizedDescriptionKey,moveError,NSUnderlyingErrorKey,nil]];
				}
				[self setTemporaryFileDownloadPath:nil];
			}
	...
	// If request has asked delegate or CSASIAuthenticationDialog for credentials
...
- (BOOL)useDataFromCache
{
	...
	if ([self cachePolicy] == CSASIReloadIfDifferentCachePolicy) {
...

{
	...
	if (![self error]) { // We may already have handled this error
		...
		[self failWithError:[NSError errorWithDomain:CSNetworkRequestErrorDomain code:CSASIConnectionFailureErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:reason,NSLocalizedDescriptionKey,underlyingError,NSUnderlyingErrorKey,nil]]];
	}
	[self checkRequestStatus];
}
...
- (id)copyWithZone:(NSZone *)zone
{
	...
	CSASIHTTPRequest *newRequest = [[[self class] alloc] initWithURL:[self url]];
...
{
	if (!sessionCookies) {
		[CSASIHTTPRequest setSessionCookies:[[[NSMutableArray alloc] init] autorelease]];
	}
...
+ (void)addSessionCookie:(NSHTTPCookie *)newCookie
{
	...
	NSHTTPCookie *cookie;
	NSUInteger i;
	NSUInteger max = [[CSASIHTTPRequest sessionCookies] count];
	for (i=0; i<max; i++) {
		cookie = [[CSASIHTTPRequest sessionCookies] objectAtIndex:i];
		if ([[cookie domain] isEqualToString:[newCookie domain]] && [[cookie path] isEqualToString:[newCookie path]] && [[cookie name] isEqualToString:[newCookie name]]) {
			[[CSASIHTTPRequest sessionCookies] removeObjectAtIndex:i];
			break;
		}
	}
	[[CSASIHTTPRequest sessionCookies] addObject:newCookie];
	[sessionCookiesLock unlock];
}
...
+ (void)clearSession
{
	...
	[[[self class] defaultCache] clearCachedResponsesForStoragePolicy:CSASICacheForSessionDurationCacheStoragePolicy];
}
...
+ (int)uncompressZippedDataFromFile:(NSString *)sourcePath toFile:(NSString *)destinationPath
{
	...
	int status = [CSASIHTTPRequest uncompressZippedDataFromSource:source toDestination:dest];
...
+ (int)compressDataFromFile:(NSString *)sourcePath toFile:(NSString *)destinationPath
{
	...
	int status = [CSASIHTTPRequest compressDataFromSource:source toDestination:dest];
...
+ (NSString *)defaultUserAgentString
{
	...
	// If we couldn't find one, we'll give up (and CSASIHTTPRequest will use the standard CFNetwork user agent)
	if (!appName) {
		return nil;
	}
	...
	// We won't bother to check for systems prior to 10.4, since CSASIHTTPRequest only works on 10.5+
    OSErr err;
...
- (void)performThrottling
{
	...
	[CSASIHTTPRequest measureBandwidthUsage];
...
+ (void)measureBandwidthUsage
{
	...
	if (!bandwidthMeasurementDate || [bandwidthMeasurementDate timeIntervalSinceNow] < -0) {
		[CSASIHTTPRequest recordBandwidthUsage];
	}
...
+ (unsigned long)maxUploadReadLength
{
	...
	// We'll split our bandwidth allowance into 4 (which is the default for an CSASINetworkQueue's max concurrent operations count) to give all running requests a fighting chance of reading data this cycle
...
{
	if (throttle) {
		[CSASIHTTPRequest throttleBandwidthForWWANUsingLimit:CSASIWWANBandwidthThrottleAmount];
	} else {
		[CSASIHTTPRequest unsubscribeFromNetworkReachabilityNotifications];
		[CSASIHTTPRequest setMaxBandwidthPerSecond:0];
		[bandwidthThrottlingLock lock];
...
{	
	...
	[CSASIHTTPRequest registerForNetworkReachabilityNotifications];	
	[bandwidthThrottlingLock unlock];
	[CSASIHTTPRequest reachabilityChanged:nil];
}
...
+ (void)registerForNetworkReachabilityNotifications
{
	[[CSReachability reachabilityForInternetConnection] startNotifier];
	[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(reachabilityChanged:) name:@"kNetworkCSReachabilityChangedNotification" object:nil];
}


+ (void)unsubscribeFromNetworkReachabilityNotifications
{
	[[NSNotificationCenter defaultCenter] removeObserver:self name:@"kNetworkCSReachabilityChangedNotification" object:nil];
}

+ (BOOL)isNetworkReachableViaWWAN
{
	return ([[CSReachability reachabilityForInternetConnection] currentReachabilityStatus] == ReachableViaWWAN);	
}

+ (void)reachabilityChanged:(NSNotification *)note
{
	[bandwidthThrottlingLock lock];
	isBandwidthThrottled = [CSASIHTTPRequest isNetworkReachableViaWWAN];
	[bandwidthThrottlingLock unlock];
}
#endif
...
+ (void)setDefaultCache:(id <CSASICacheDelegate>)cache
{
	[defaultCache release];
	defaultCache = [cache retain];
}

+ (id <CSASICacheDelegate>)defaultCache
{
	return defaultCache;
}
...
+ (NSThread *)threadForRequest:(CSASIHTTPRequest *)request
{
	if (!networkThread) {
