...
extern NSString *CSASIHTTPRequestVersion;
...
typedef enum _CSASIAuthenticationState {
	CSASINoAuthenticationNeededYet = 0,
	CSASIHTTPAuthenticationNeeded = 1,
	CSASIProxyAuthenticationNeeded = 2
} CSASIAuthenticationState;

typedef enum _CSASINetworkErrorType {
    CSASIConnectionFailureErrorType = 1,
    CSASIRequestTimedOutErrorType = 2,
    CSASIAuthenticationErrorType = 3,
    CSASIRequestCancelledErrorType = 4,
    CSASIUnableToCreateRequestErrorType = 5,
    CSASIInternalErrorWhileBuildingRequestType  = 6,
    CSASIInternalErrorWhileApplyingCredentialsType  = 7,
	CSASIFileManagementError = 8,
	CSASITooMuchRedirectionErrorType = 9,
	CSASIUnhandledExceptionError = 10
	
} CSASINetworkErrorType;

// The error domain that all errors generated by CSASIHTTPRequest use
extern NSString* const CSNetworkRequestErrorDomain;
...
extern unsigned long const CSASIWWANBandwidthThrottleAmount;

@interface CSASIHTTPRequest : NSOperation <NSCopying> {
	...
	id <CSASIHTTPRequestDelegate> delegate;
	
	// Another delegate that is also notified of request status changes and progress updates
	// Generally, you won't use this directly, but CSASINetworkQueue sets itself as the queue so it can proxy updates to its own delegates
	id <CSASIHTTPRequestDelegate, CSASIProgressDelegate> queue;
	...
	// Automatically set to true in CSASIFormDataRequests when using setFile:forKey:
	...
	// Set to true when CSASIHTTPRequest automatically created a temporary file containing the request body (when true, the file at postBodyFilePath will be deleted at the end of the request)
	...
	// If error code is = CSASIConnectionFailureErrorType (1, Connection failure occurred) - inspect [[error userInfo] objectForKey:NSUnderlyingErrorKey] for more information
	NSError *error;
	...
	id <CSASIProgressDelegate> uploadProgressDelegate;
	...
	id <CSASIProgressDelegate> downloadProgressDelegate;
	...
	// When YES, CSASIHTTPRequest will present a dialog allowing users to enter credentials when no-matching credentials were found for a server that requires authentication
	...
	// When YES, CSASIHTTPRequest will present a dialog allowing users to enter credentials when no-matching credentials were found for a proxy server that requires authentication
	...
	// If you set this and implement the method in your delegate, you must handle the data yourself - CSASIHTTPRequest will not populate responseData or write the data to downloadDestinationPath
	...
	CSASIHTTPRequest *mainRequest;
	
	// When NO, this request will only update the progress indicator when it completes
	// When YES, this request will update the progress indicator according to how much data it has received so far
	// The default for requests is YES
	// Also see the comments in CSASINetworkQueue.h
	...
	// Tells CSASIHTTPRequest not to delete partial downloads, and allows it to use an existing file to resume a download. Defaults to NO.
	...
	// Details on the proxy to use - you could set these yourself, but it's probably best to let CSASIHTTPRequest detect the system proxy settings
	NSString *proxyHost;
	int proxyPort;
	
	// CSASIHTTPRequest will assume kCFProxyTypeHTTP if the proxy type could not be automatically determined
	...
	// See CSASIAuthenticationState values above. 0 == default == No authentication needed yet
	CSASIAuthenticationState authenticationNeeded;
	
	// When YES, CSASIHTTPRequests will present credentials from the session store for requests to the same server before being asked for them
	...
	// Will be CSASIHTTPRequestRunLoopMode for synchronous requests, NSDefaultRunLoopMode for all other requests
	...
	// The download cache that will be used for this request (use [CSASIHTTPRequest setDefaultCache:cache] to configure a default cache
	id <CSASICacheDelegate> downloadCache;
	
	// The cache policy that will be used for this request - See CSASICacheDelegate.h for possible values
	CSASICachePolicy cachePolicy;
	
	// The cache storage policy that will be used for this request - See CSASICacheDelegate.h for possible values
	CSASICacheStoragePolicy cacheStoragePolicy;
...
// Used internally to record bandwidth use, and by CSASIInputStreams when uploading. It's probably best if you don't mess with this.
+ (void)incrementBandwidthUsedInLastSecond:(unsigned long)bytes;

// On iPhone, CSASIHTTPRequest can automatically turn throttling on and off as the connection type changes between WWAN and WiFi
...
@property (assign) CSASIHTTPRequest *mainRequest;
...
@property (assign, readonly) CSASIAuthenticationState authenticationNeeded;
...
@property (assign) id <CSASICacheDelegate> downloadCache;
@property (assign) CSASICachePolicy cachePolicy;
@property (assign) CSASICacheStoragePolicy cacheStoragePolicy;
