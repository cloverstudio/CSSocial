...
static CSASIAuthenticationDialog *sharedDialog = nil;
...
@implementation CSASIAutorotatingViewController
...
@interface CSASIAuthenticationDialog ()
- (void)showTitle;
- (void)show;
...
@implementation CSASIAuthenticationDialog

#pragma mark init / dealloc

+ (void)initialize
{
	if (self == [CSASIAuthenticationDialog class]) {
...
+ (void)presentAuthenticationDialogForRequest:(CSASIHTTPRequest *)request
{
	...
	if (!sharedDialog) {
		...
		if ([request authenticationNeeded] == CSASIProxyAuthenticationNeeded) {
			[sharedDialog setType:CSASIProxyAuthenticationType];
		} else {
			[sharedDialog setType:CSASIStandardAuthenticationType];
		}
		[sharedDialog show];
	} else {
...
- (UIViewController *)presentingController
{
	if (!presentingController) {
		presentingController = [[CSASIAutorotatingViewController alloc] initWithNibName:nil bundle:nil];
...
- (void)showTitle
{
	...
	if (UIInterfaceOrientationIsPortrait([[UIDevice currentDevice] orientation])) {
		// Setup the title
		if ([self type] == CSASIProxyAuthenticationType) {
...
{
	for (CSASIHTTPRequest *theRequest in [self requestsRequiringTheseCredentials]) {
...
- (NSArray *)requestsRequiringTheseCredentials
{
	...
	for (CSASIHTTPRequest *otherRequest in requestsNeedingAuthentication) {
...
- (void)presentNextDialog
{
	if ([requestsNeedingAuthentication count]) {
		CSASIHTTPRequest *nextRequest = [requestsNeedingAuthentication objectAtIndex:0];
...
- (void)loginWithCredentialsFromDialog:(id)sender
{
	for (CSASIHTTPRequest *theRequest in [self requestsRequiringTheseCredentials]) {
		...
		// Handle NTLM domains
		NSString *scheme = ([self type] == CSASIStandardAuthenticationType) ? [[self request] authenticationScheme] : [[self request] proxyAuthenticationScheme];
