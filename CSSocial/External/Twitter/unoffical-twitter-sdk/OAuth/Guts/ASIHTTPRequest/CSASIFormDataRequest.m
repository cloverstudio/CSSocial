...
// Private stuff
@interface CSASIFormDataRequest ()
...
@implementation CSASIFormDataRequest
...
- (id)initWithURL:(NSURL *)newURL
{
	...
	[self setPostFormat:CSASIURLEncodedPostFormat];
...
- (void)addFile:(id)data withFileName:(NSString *)fileName andContentType:(NSString *)contentType forKey:(NSString *)key
{
	...
	if ([data isKindOfClass:[NSString class]]) {
		...
		if (!fileExists || isDirectory) {
			[self failWithError:[NSError errorWithDomain:CSNetworkRequestErrorDomain code:CSASIInternalErrorWhileBuildingRequestType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"No file exists at %@",data],NSLocalizedDescriptionKey,nil]]];
		}
		...
		if (!contentType) {
			contentType = [CSASIHTTPRequest mimeTypeForFileAtPath:data];
		}
	}
...
- (void)buildPostBody
{
	...
	if ([self postFormat] == CSASIURLEncodedPostFormat) {
...
- (void)buildURLEncodedPostBody
{
	...
	if ([[self fileData] count] > 0) {
		[self setPostFormat:CSASIMultipartFormDataPostFormat];
		[self buildMultipartFormDataPostBody];
		return;
	}
...
{
	CSASIFormDataRequest *newRequest = [super copyWithZone:zone];
