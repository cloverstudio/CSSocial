...
#import "CSASIHTTPRequestConfig.h"

typedef enum _CSASIPostFormat {
    CSASIMultipartFormDataPostFormat = 0,
    CSASIURLEncodedPostFormat = 1
	
} CSASIPostFormat;

@interface CSASIFormDataRequest : CSASIHTTPRequest <NSCopying> {
	...
	CSASIPostFormat postFormat;
	
	NSStringEncoding stringEncoding;
	
#if DEBUG_FORM_DATA_REQUEST
	// Will store a string version of the request body that will be printed to the console when CSASIHTTPREQUEST_DEBUG is set in GCC_PREPROCESSOR_DEFINITIONS
...
@property (assign) CSASIPostFormat postFormat;
